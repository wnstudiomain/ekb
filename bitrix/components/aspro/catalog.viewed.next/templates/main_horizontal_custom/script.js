			...
			if(typeof BX.localStorage !== 'undefined'){
				...
				for(var PRODUCT_ID in arViewedLocal){
					...
					if($item.length){
						...
						var tmpMiniPrice = $('#bx_1675152901_'+item.PRODUCT_ID).attr('data-mini');
						console.log(tmpMiniPrice);
						var minPriceOld = item.MIN_PRICE.PRINT_DISCOUNT_VALUE;
						console.log(minPriceOld);
						var minPrice = (item.MIN_PRICE.DISCOUNT_VALUE * tmpMiniPrice).toFixed(0);
						console.log(minPrice);
						if(minPrice>0){
							resMinPrice = minPrice + ' руб.';
							resMeasure = tmpMiniPrice + ' гр.';
						}else{
							resMinPrice = item.MIN_PRICE.PRINT_DISCOUNT_VALUE;
							resMeasure = item.CATALOG_MEASURE_NAME + '.';
						}
						$item.html(
									...
									'<div class="cost prices clearfix">'+
										(item.MIN_PRICE ?
											((((item.MIN_PRICE.VALUE * 1) > (item.MIN_PRICE.DISCOUNT_VALUE * 1))) ?
												'<div class="price only_price">' + (bWithOffers ? BX.message('CATALOG_FROM_VIEWED')+' ' : '') + resMinPrice + (bShowMeasure && item.CATALOG_MEASURE_NAME.length ? '/' + resMeasure : '') + '</div>'
											: '<div class="price only_price">' + (bWithOffers ? BX.message('CATALOG_FROM_VIEWED')+' ' : '') + resMinPrice + (bShowMeasure && item.CATALOG_MEASURE_NAME.length ? '/' + resMeasure : '') + '</div>')
										: '')+
									'</div>'+
								'</div>'+
