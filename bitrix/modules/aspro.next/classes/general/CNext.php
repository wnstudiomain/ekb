...
?>
		...
		<div class="contacts">
			...
			<? else : ?>
				<div class="address">
					...
					<? $APPLICATION->IncludeFile(
						SITE_DIR . "include/top_page/site-address.php",
						array(),
						array(
							"MODE" => "html",
							"NAME" => "Address",
			...
			<? else : ?>
				<div class="email">
					<?= self::showIconSvg("email", SITE_TEMPLATE_PATH . "/images/svg/Email.svg"); ?>
					<? $APPLICATION->IncludeFile(
						SITE_DIR . "include/footer/site-email.php",
						array(),
						array(
							"MODE" => "html",
							"NAME" => "Address",
	...
	public static function ShowMobileRegions()
	{
		...
		if ($arRegion) :
			...
			$arRegion['CURRENT_REGION'] = CNextRegionality::getCurrentRegion();
			$arRegion['REAL_REGION'] = CNextRegionality::getRealRegionByIP();
			$city = $arRegion['REAL_REGION']['ID'];
			$arRegion['REGION_SELECTED'] = isset($_COOKIE['current_region']) && $_COOKIE['current_region'];
			$arRegions = CNextRegionality::getRegions();
			$regionID = ($arRegion ? $arRegion['ID'] : '');
			$iCountRegions = count($arRegions);
			if ($arRegion['REGION_SELECTED'] == '') {
				SetCookie("current_region", $city);
			}
			//SetCookie("current_region",$city);
		?>
					...
					<li>
						<? if (self::GetFrontParametrValue('REGIONALITY_SEARCH_ROW') != 'Y') : ?>
							...
							<? else : ?>
								...
								<span><?= ($arRegion['REGION_SELECTED'] == '') ? $arRegion['REAL_REGION']['NAME'] : $arRegion['CURRENT_REGION']['NAME']; ?></span>
								<? if ($iCountRegions > 1) : ?>
									<span class="arrow"><i class="svg svg_triangle_right"></i></span>
								<? endif; ?>
								</a>
	...
	public static function ShowAdminRow($optionCode, $arOption, $arTab, $arControllerOption)
	{
        ...
        return '<style>'.$style.'</style>';
				...
				public static function get_banners_position($position, $show_all = 'N')
				{
					...
					if ($arTheme["ADV_" . $position] == 'Y') {
						...
						$APPLICATION->IncludeComponent(
							...
							false,
							array('ACTIVE_COMPONENT' => 'Y', 'HIDE_ICONS' => 'Y')
						);
					}
				}
				...
				public static function showPriceMatrix($arItem = array(), $arParams, $strMeasure = '', $arAddToBasketData = array())
				{
					...
					if (isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']) {
						...
						<div class="price_matrix_block">
							...
							$item_id = $arItem["ID"];
							$prop = CIBlockElement::GetByID($item_id)->GetNextElement()->GetProperties();
							if ($prop['WEIGHTED_GOODS']['~VALUE'])
								$minQuantity = $prop['MINIMAL_COUNT_GOODS']['VALUE'];
							else
								$minQuantity = 1;
							if ($sDiscountPrices)
											...
											<? if ($iCountPriceInterval > 1) : ?>
												...
												<? if ($arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]) { ?>
													<div class="price new-price" data-currency="<?= $arPrice["CURRENCY"]; ?>" data-value="<?= $arPrice["DISCOUNT_PRICE"]; ?>">
														<? if (strlen($arPrice["DISCOUNT_PRICE"])) : ?>
															...
															<? elseif (($arParams["SHOW_MEASURE"] == "Y") && $strMeasure && $arPrice["PRICE"]) : ?>
													...
													<? if ($arParams["SHOW_OLD_PRICE"] == "Y") : ?>
														<div class="price discount" data-currency="<?= $arPrice["CURRENCY"]; ?>" data-value="<?= $arPrice["PRICE"]; ?>">
															<span class="values_wrapper"><?= \Aspro\Functions\CAsproItem::getCurrentPrice("PRICE", $arPrice, $minQuantity); ?></span>
															<? if (($arParams["SHOW_MEASURE"] == "Y") && $strMeasure && $arPrice["PRICE"] && $minQuantity > 1) : ?>
																<span class="price_measure">/<?= $minQuantity ?> <?= $strMeasure ?></span>
															<? elseif (($arParams["SHOW_MEASURE"] == "Y") && $strMeasure && $arPrice["PRICE"]) : ?>
																<span class="price_measure">/<?= $strMeasure ?></span>
												...
												<? else :
													...
													<div class="sale_block">
														...
														<div class="text"><span class="values_wrapper" data-currency="<?= $arPrice["CURRENCY"]; ?>" data-value="<?= (($arPrice["PRICE"] - $arPrice["DISCOUNT_PRICE"]) * $ratio); ?>"><?= CAllCurrencyLang::CurrencyFormat(($arPrice["PRICE"] - $arPrice["DISCOUNT_PRICE"]), $arPrice['CURRENCY']) ?></span></div>
													</div>
												<? endif; ?>
											<? endif; ?>
										<? endforeach; ?>
				...
				public static function showPriceRangeTop($arItem, $arParams, $strMeasure, $mess = '')
				{
					$html = '';
					if ($arItem) {
						if (isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']) {
							...
							<div class="with_matrix">
								<? if ($arParams['SHOW_OLD_PRICE'] == 'Y' && $arItem['ITEM_PRICES'][$priceSelected]['BASE_PRICE'] > $arItem['ITEM_PRICES'][$priceSelected]['PRICE']) : ?>
									<div class="price new-price">
										<div class="price_value_block"><?= \Aspro\Functions\CAsproItem::getCurrentPrice("PRICE", $arItem['ITEM_PRICES'][$priceSelected], $minQuantity); ?></div>
										<? if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME'] && $arItem['ITEM_PRICES'][$priceSelected]["PRICE"] && $minQuantity > 1) : ?>
											<span class="price_measure">/<?= $minQuantity ?> <?= $strMeasure ?></span>
										<? else : ?>
											<span class="price_measure">/<?= $strMeasure ?></span>
										<? endif; ?>
									</div>
									<div class="price discount"><span class="values_wrapper"><?= \Aspro\Functions\CAsproItem::getCurrentPrice("BASE_PRICE", $arItem['ITEM_PRICES'][$priceSelected], $minQuantity); ?></span>
										<? if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME'] && $arItem['ITEM_PRICES'][$priceSelected]["PRICE"] && $minQuantity > 1) : ?>
											<span class="price_measure">/<?= $minQuantity ?> <?= $strMeasure ?></span>
										<? else : ?>
											<span class="price_measure">/<?= $strMeasure ?></span>
								...
								<? if ($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y' && $arItem['ITEM_PRICES'][$priceSelected]['DISCOUNT']) : ?>
									<div class="sale_block matrix">
										<div class="sale_wrapper">
											...
											<div class="text">
												<span class="values_wrapper"><?= \Aspro\Functions\CAsproItem::getCurrentPrice("DISCOUNT", $arItem['ITEM_PRICES'][$priceSelected], $minQuantity); ?></span>
												<? if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME'] && $arItem['ITEM_PRICES'][$priceSelected]["PRICE"] && $minQuantity > 1) : ?>
													<span class="price_measure">/<?= $minQuantity ?> <?= $strMeasure ?></span>
												<? else : ?>
													<span class="price_measure">/<?= $strMeasure ?></span>
		...
		");
				...
				public static function GetItemsIDs($arItem, $detail = "N")
				{
					...
					$arAllIDs["TITLE_ITEM"] = (isset($arItem['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE']) && $arItem['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE'] != ''
						...
						foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer) {
							...
							if (!empty($arJSOffer['DISPLAY_PROPERTIES'])) {
								foreach ($arJSOffer['DISPLAY_PROPERTIES'] as $arOneProp) {
									...
									$strProps .= '<tr itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue"><td><span itemprop="name">' . $arOneProp['NAME'] . '</span></td><td><span itemprop="value">' . (is_array($arOneProp['VALUE'])
										? implode(' / ', $arOneProp['VALUE'])
										: $arOneProp['VALUE']) . '</span></td></tr>';
								}
							}
							if ($arArticle) {
								$strArticle = '';
								$strArticle .= $arArticle['NAME'] . ': ' . (is_array($arArticle['VALUE'])
						...
						if (
							...
							if ('MORE_PHOTO' == $propertyCode && isset($item['MORE_PHOTO']) && !empty($item['MORE_PHOTO'])) {
								...
								if (
									...
									$fileValues = (isset($item['DISPLAY_PROPERTIES'][$propertyCode]['FILE_VALUE']['ID']) ?
				...
				public static function GetCurrentSectionSubSectionFilter(&$arVariables, &$arParams, $CurrentSectionID = false)
				{
					...
					if (!$arFilter['SECTION_ID'] = ($CurrentSectionID !== false ? $CurrentSectionID : ($arVariables['SECTION_ID'] ? $arVariables['SECTION_ID'] : false))) {
						...
						array_merge($arFilter, array('INCLUDE_SUBSECTIONS' => 'N', 'DEPTH_LEVEL' => '1'));
