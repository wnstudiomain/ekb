...
if (empty($arResult['ERROR_MESSAGE']))
{
	...
	<script>
		...
		if ($(window).width() < 767) {
			$('.basket-item-block-info').append('<div class="basket-item-block-offer"></div>')
		$('.basket-items-list-item-container').each(function(index, element) {
			var basketInfo = $(element).find('.basket-item-block-offer'),
				basketCount = $(element).find('.basket-item-block-amount'),
				basketPrice =  $(element).find('td:eq(3) .basket-item-block-price')
				basketCount.detach().appendTo(basketInfo);
				basketPrice.detach().appendTo(basketInfo);
				$(element).find('td.basket-items-list-item-amount').detach()
				$(element).find('td.basket-items-list-item-price').detach()
		})
		}

		//$('.basket-item-block-amount').detach().appendTo('.basket-item-block-info')
	</script>
	<?
