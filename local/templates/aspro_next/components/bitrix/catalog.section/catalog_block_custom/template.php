...
<?endif;?>
<div
	class="top_wrapper row margin0 <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>">
		...
		<div class="item_block col-<?=$col;?> col-md-<?=ceil(12/$col);?> col-sm-<?=ceil(12/round($col / 2))?> col-xs-6">
			<div class="catalog_item_wrapp item">
								...
								<td>
									...
									<label>
										<input type="radio"
											...
											value="<? echo $valueID; ?>" <? echo ($valueID==$propInfo['SELECTED']
											? '"checked"' : '' ); ?>>
										<? echo $value; ?>
									</label>
									<?}
									...
									<select name="<? echo $arParams['PRODUCT_PROPS_VARIABLE']; ?>[<? echo $propID; ?>]">
										<?
														foreach($propInfo['VALUES'] as $valueID => $value){?>
										<option value="<? echo $valueID; ?>" <? echo ($valueID==$propInfo['SELECTED']
											? '"selected"' : '' ); ?>>
											<? echo $value; ?>
										</option>
										<?}?>
				...
				<?$this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arParams["IBLOCK_ID"], "ELEMENT_EDIT"));
					...
					$strMeasure = '';
					$arAddToBasketData;
					$minimalCountGoods = $arItem['PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'];
					if(!$arItem["OFFERS"] || $arParams['TYPE_SKU'] !== 'TYPE_1'){
						...
						if(!$arItem['PROPERTIES']['WEIGHTED_GOODS']['~VALUE']):
							...
							$arAddToBasketData = \Aspro\Functions\CAsproNextCustom::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], ($bLinkedItems ? true : false), $arItemIDs["ALL_ITEM_IDS"], 'small', $arParams, $minimalCountGoods);
						endif;
					}
				...
				<div class="catalog_item main_item_wrapper item_wrap <?=(($_GET['q'])) ? 's' : ''?>"
					...
					<div>
						<div class="image_wrapper_block">
							...
							<div class="like_icons">
								...
								<div class="wish_item_button"
									<?=($arAddToBasketData['CAN_BUY'] ? '' : 'style="display:none"');?>>
								...
								<div class="compare_item_button">
									<span title="<?=GetMessage('CATALOG_COMPARE')?>"
										...
										data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item=""><i></i></span>
									<span title="<?=GetMessage('CATALOG_COMPARE_OUT')?>"
										class="compare_item in added <?=$arParams["TYPE_SKU"];?>" style="display: none;"
										data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item=""><i></i></span>
								</div>
								<?endif;?>
								<?endif;?>
							...
							<?endif;?>
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine" id="<? echo $arItemIDs["
								ALL_ITEM_IDS"]['PICT']; ?>">
								<?
						...
						<div class="item_info <?=$arParams["TYPE_SKU"]?>">
							...
							<div class="cost prices clearfix">
								...
								<?else:?>
								<?$strMeasureCustom = $arItem['PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'].' '.$strMeasure;?>
								<?=\Aspro\Functions\CAsproNextCustom::showPriceMatrix($arItem, $arParams, $strMeasureCustom, $arAddToBasketData, $minimalCountGoods);?>
								<?endif;?>
							...
							<div class="quantity_block">
								...
								<div class="values">
									<span class="item">
										<span class="value"
											<?=((count( $arItem["OFFERS"] ) > 0 && $arParams["TYPE_SKU"] == 'TYPE_1' && $arItem["OFFERS_PROP"]) ? 'style="opacity:0;"' : '')?>><?=$totalCount;?></span>
					...
					<div class="footer_button">
						<div class="sku_props">
							...
							<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["
								ALL_ITEM_IDS"]['PROP_DIV']; ?>">
								<?$arSkuTemplate = array();?>
							...
							<script type="text/javascript">
								var < ? echo $arItemIDs["strObName"]; ? > = new JCCatalogSection( < ? echo CUtil::PhpToJSObject($arItemJSParams,
									false, true); ? > );
							</script>
							<?}?>
							<?}?>
                    ...
                    <div class="counter_block big_basket" data-offers="<?=($arResult["OFFERS"] ? "Y" : "N");?>"
                        ...
                        <?else:?>
                        <span class="minus-weight" id="<? echo $arItemIDs[" ALL_ITEM_IDS"]['QUANTITY_DOWN'];
                            ?>">-</span>
                        <?endif;?>
                    ...
                    <div id="<? echo $arItemIDs[" ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block
						...
						<script type="text/javascript">
							var < ? echo $arItemIDs["strObName"]; ? > el = new JCCatalogSectionOnlyElement( < ? echo CUtil::PhpToJSObject(
