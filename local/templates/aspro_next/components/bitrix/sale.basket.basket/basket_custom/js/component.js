  ...
  BX.Sale.BasketComponent = {
    ...
    bindInitialEvents: function () {
      ...
      BX.bind(
        window,
        "resize",
        BX.throttle(this.adjustBasketWrapperHeight, 20, this)
      );
    },
    ...
    adjustBasketWrapperHeight: function () {
      ...
      if (BX.type.isDomNode(itemListContainer) && BX.type.isDomNode(itemList)) {
        if (
          itemListContainer.clientHeight + this.stickyHeaderOffset >
          window.innerHeight
        ) {
          itemListContainer.style.minHeight =
            "calc(100vh - 15px - " + this.stickyHeaderOffset + "px)";
          itemList.style.minHeight =
            "calc(100vh - 15px - " + this.stickyHeaderOffset + "px)";
        } else {
          itemListContainer.style.minHeight =
            itemListContainer.clientHeight + "px";
          ...
          if (hashMap[hash].length > 1) {
            for (var i = 0; i < hashMap[hash].length; i++) {
              ...
              if (
                ...
                this.items[
                  hashMap[hash][i]
                ].SIMILAR_ITEMS_QUANTITY = otherSimilarItemsQuantity;
                this.items[
                  hashMap[hash][i]
                ].TOTAL_SIMILAR_ITEMS_QUANTITY = totalSimilarItemsQuantity;

                this.items[
                  hashMap[hash][i]
                ].ALL_AVAILABLE_QUANTITY = this.items[
                  hashMap[hash][i]
                ].AVAILABLE_QUANTITY;
            ...
            this.items[hashMap[hash][0]].AVAILABLE_QUANTITY = this.items[
              hashMap[hash][0]
            ].ALL_AVAILABLE_QUANTITY;
    ...
    checkBasketItemsAnimation: function (itemData) {
      ...
      if (this.items[itemId]) {
        ...
        if (
          ...
          //console.log(itemData.SUM_PRICE, itemData.SUM_PRICE_NEW)
    ...
    getSkuPropertyValues: function (basketItemNode) {
      ...
      for (var i = 0, l = propNodes.length; i < l; i++) {
        propertyValues[
          propNodes[i].getAttribute("data-property")
