...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	<div class="top_wrapper items_wrapper">
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$minimalCountGoods = $arItem['DISPLAY_PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'];
			if(!$arItem['DISPLAY_PROPERTIES']['WEIGHTED_GOODS']['~VALUE']):
				...
				$arAddToBasketData = \Aspro\Functions\CAsproNextCustom::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, array(), 'small', $arParams, $minimalCountGoods);
			endif;
			...
			<div class="catalog_item_wrapp col-m-20 col-lg-<?=$col;?> col-md-4 col-sm-<?=floor(12 / round($arParams['LINE_ELEMENT_COUNT'] / 2))?> col-xs-<?=floor(12 / round($arParams['LINE_ELEMENT_COUNT'] / 2))?> item" data-col="<?=$col;?>">
				<div class="catalog_item item_wrap " id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<div class="inner_wrap">
						<div class="image_wrapper_block shine">
                            ...
                            <?if ($arItem["ITEM_PRICES"][0]["BASE_PRICE"] > $arItem["ITEM_PRICES"][0]["PRICE"]):?>
                                <div class="sale_percent">
                                    <?$percentSale = 100 - round(($arItem["ITEM_PRICES"][0]["PRICE"]/$arItem["ITEM_PRICES"][0]["BASE_PRICE"])*100, 2) ?>
						...
						<div class="item_info">
							...
							<div class="cost prices clearfix">
								...
								<?}else{?>
									<?
									$item_id = $arItem["ID"];
									$arItem['PROPERTIES'] = CIBlockElement::GetByID($item_id)->GetNextElement()->GetProperties();
									if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']) // USE_PRICE_COUNT
									{?>
										<?if($arItem['ITEM_PRICE_MODE'] == 'Q' && count($arItem['PRICE_MATRIX']['ROWS']) > 1):?>
											<?=CNext::showPriceRangeTop($arItem, $arParams, $strMeasure);?>

										<?elseif(!$arItem['DISPLAY_PROPERTIES']['WEIGHTED_GOODS']['~VALUE']):?>

											<?=CNext::showPriceMatrix($arItem, $arParams, $strMeasure, $arAddToBasketData);?>
										<?else:?>
											<?$strMeasureCustom = $arItem['DISPLAY_PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'].' '.$strMeasure;?>
