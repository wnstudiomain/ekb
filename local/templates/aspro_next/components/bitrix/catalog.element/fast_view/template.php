		...
		<div class="middle_info main_item_wrapper">
					...
					<?}else{?>
						...
						<?if($arResult['PROPERTIES']['WEIGHTED_GOODS']['~VALUE']):?>
							...
							<?else:?>
								<?$minQuantity = 1 ?>
							<?endif;?>
							<?\Aspro\Functions\CAsproItem::showItemPrices($arParams, $arResult["PRICES"], $minQuantity, $strMeasure, $min_price_id, 'Y');?>
			...
			<div class="buy_block">
				...
				<?if(!$arResult["OFFERS"]):?>
                    ...
                    <div class="counter_block counter_block_fast" data-offers="<?=($arResult["OFFERS"] ? "Y" : "N");?>"
                        data-item="<?=$arResult["ID"];?>"
                        <?=(($arResult["OFFERS"] && $arParams["TYPE_SKU"]=="N") ? "style='display: none;'" : "");?>>
                        <?if(!$arResult['PROPERTIES']['WEIGHTED_GOODS']['~VALUE']):?>
                        <span class="disable minus minus-button" id="<? echo $arItemIDs[" ALL_ITEM_IDS"]['QUANTITY_DOWN']; ?>">-</span>
                        <?else:?>
                        <span class="disable minus-weight minus-button" data-mini_count="<?=$arResult['PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'] ?>">-</span>
						<?endif;?>
						<?$inputValue = $arResult['PROPERTIES']['WEIGHTED_GOODS']['~VALUE'] ? $arResult['PROPERTIES']['MINIMAL_COUNT_GOODS']['VALUE'] : $arAddToBasketData["MIN_QUANTITY_BUY"]?>
