		...
		*/
			...
			node.appendChild(
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {
								className: 'bx-soa-more-btn col-xs-12'
							},
							children: buttons
						})
					]
			...
			authContent.appendChild(
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {
								className: 'bx-soa-reference col-xs-12'
							},
							children: [
		...
		editBasketItemsHeader: function (basketItemsNode) {
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {
						className: 'bx-soa-item-tr hidden-sm hidden-xs'
					},
					children: headers
				})
			);
		...
		editActivePaySystemBlock: function (activeNodeMode) {
			...
			if ($(window).width() > 1000) {
				...
				$('#bx-soa-paysystem .bx-soa-pp .bx-soa-pp-company').each(function () {
					var curHeight = $(this).find('.bx-soa-pp-company-graf-container').height();
					if (curHeight > maxHeight) {
						maxHeight = curHeight;
					}
				});

				$('#bx-soa-paysystem .bx-soa-pp .bx-soa-pp-company .bx-soa-pp-company-graf-container').height(maxHeight);
			}
		},
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', {
					props: {
						className: 'bx-soa-pp-item-container'
					}
				}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function (item) {
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				...
				title = BX.create('DIV', {
					...
					html: "<div class='order-title'>" + item.NAME + "</div>" + "<div class='order-description'>" + item.DESCRIPTION + "</div>",
				});
			}
			...
			itemNode = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company col-md-4'
				},
				children: [label],
				events: {
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			var deliveryInfoContainer = BX.create('DIV', {
					props: {
						className: 'col-sm-5 bx-soa-pp-desc-container'
					}
				}),
		...
		createDeliveryItem: function (item) {
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, span, currentDelivery;
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
				...
				if (item.PERIOD_TEXT && item.PERIOD_TEXT.length && item.PERIOD_TEXT !== 'undefined') {
					var periodDelivery = item.PERIOD_TEXT;
				} else {
					var periodDelivery = '';
				}
				...
				if (item.ID == 2) {
					if ('CHECKED' in item) {
						...
						if (prop7 !== null) {
							prop7.value = 'ул.Красных командиров, д.16, оф.1';
						}
					}
		...
		editFadeDeliveryContent: function (node) {
			...
			if (selectedDelivery && selectedDelivery.NAME) {
				...
				node.appendChild(
					BX.create('DIV', {
						props: {
							className: 'col-sm-3 bx-soa-pp-price'
						},
		...
		editTotalBlock: function () {
			...
			if (!this.result.SHOW_AUTH) {
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {
							className: 'bx-soa-cart-total-button-container' + (!showOrderButton ? ' visible-xs' : '')
						},
						children: [
							BX.create('A', {
								props: {
		...
		totalBlockScrollCheck: function () {
			...
			var scrollTop = BX.GetWindowScrollPos().scrollTop,
				ghostTop = BX.pos(this.totalGhostBlockNode).top - 90,
