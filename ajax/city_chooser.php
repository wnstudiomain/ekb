...
<div class="form">
	...
	<script type="text/javascript">
		...
		if(arNextOptions['THEME']['REGIONALITY_SEARCH_ROW'] != 'Y')
		{
			$("#search").autocomplete({
				...
				select: function(event, ui) {
					...
					$.cookie('current_location', ui.item.LOCATION, {path: '/',domain: arNextOptions['SITE_ADDRESS']});
	    ...
	    $('.popup_regions .item a').on('click', function(e){
	    	e.preventDefault();
	    	var _this = $(this);
			$.removeCookie('current_region');
			$.removeCookie('current_location');
			$.cookie('current_region', _this.data('id'), {path: '/',domain: arNextOptions['SITE_ADDRESS']});
			$.cookie('current_location', _this.data('location'), {path: '/',domain: arNextOptions['SITE_ADDRESS']});
