...
$(document).ready(function () {
	...
	$(".nav-tabs").on("click","a", function (event) {
		...
		$(".nav-tabs").attr('data-clicked', 'yes');
        var id  = $(this).attr('href'),
			top = $(id).offset().top - 140;
			$('li.active').removeClass('active');
			$(this).parent().addClass('active');
		$('body,html').animate({scrollTop: top}, 600, 'swing'  , function() {
			$(".nav-tabs").attr('data-clicked', 'no');
		  });
	});

	$('#discount-block__button').click(function(e){
		e.stopPropagation();
		$('.discount-descp').toggleClass('active')
	})

	$('html').click(function() {
		$('.discount-descp').removeClass('active')
	 });

	$('.discount-descp').click(function(e) {
		e.stopPropagation();
	})
	...
	var scrollNav = function () {
		$(window).on('scroll', function () {
			...
			if ($(".nav-tabs").attr('data-clicked') == 'no') {
				$sections.each(function(i,el){
					if (i === 0) {
						var top  = $(el).offset().top - 150;
					}
					else {
						var top  = $(el).offset().top-270;
					}
					...
					else if (tabs.position().top === 0 && ($(".nav-tabs").attr('data-clicked') == 'no')) {
						$('li.active').removeClass('active');
					}
				})
			}
	...
	$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
		...
		if (_this.closest('.product-item-detail-tabs-list').length) {
			...
			$('html, body').animate({
				scrollTop: content_offset.top - hfixed_height - tab_height
			}, 400);
		}
			...
			if (eventdata) {
				...
				if (slider) {
					...
					if (slider.hasClass('small_slider')) {
						$('.detail .small-gallery-block .item').sliceHeight({
							lineheight: -3
						});
					}
					if (slider.hasClass('big_slider')) {
						$('.detail .big_slider .item').sliceHeight({
							lineheight: -3
						});
					}
				}
	...
	window.JCCatalogElement.prototype.ChangeInfo = function () {
		...
		if (-1 < index) {
			...
			if ($(this.obProduct).find('.quantity_block .values').length)
				$(this.obProduct).find('.quantity_block .values .item span.value').text(this.offers[index].MAX_QUANTITY).css({
					'opacity': '1'
				});
