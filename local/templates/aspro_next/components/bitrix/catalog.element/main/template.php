...
<? /* ==================== edost КОНЕЦ */ ?>
    ...
    <div class="help_block">
        <div class="help_block_item">
            Отзывы<span class="ask_icon goto_review">?</span>
        </div>
    </div>
<style>
    .help_block {
    ...
    .help_block_item:before{
        background-position: -90px 0 !important;
        background-color: #622e16 !important;
        content: "";
        position: absolute;
        left: 10px;
        top: 7px;
        ...
        width: 30px;
        height: 30px;
        background: url('/local/templates/aspro_next/images/order.png') 0 0 no-repeat;
    }
    span.ask_icon {
        font-weight: bold;
        background: #622e1640;
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('.goto_review').click( function(e){
            e.preventDefault();
            console.log('mouse'); // ловим клик по ссылке с классом go_to
            //var scroll_el = $(this).attr('href'); // возьмем содержимое атрибута href, должен быть селектором, т.е. например начинаться с # или .
            //if ($(scroll_el).length != 0) { // проверим существование элемента чтобы избежать ошибки
            $('html, body').animate({ scrollTop: $('.tabs_section .product_reviews_tab a[href="#review"]').offset().top-80 }, 1800); // анимируем скроолинг к элементу scroll_el
            //}
